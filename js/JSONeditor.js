JSONeditor={
	start:function(treeDivName,formDivName,json,showExamples){
		...
		fs.fontFamily=s.fontFamily="Roboto,Verdana,Arial,Helvetica,sans-serif"
...
function prettyPrint(ugly) {
	...
	try{
		...
		if(!outerException instanceof SyntaxError){
			alert("Invalid");
		} else {
			obj = ugly;
		}
    ...
    document.getElementById('myText').value = pretty;
    document.getElementById('myText').scrollTop = 0;
	return obj;
}
var notify;
function generateNotifyProgressBar(){
	notify = $.notify({
...
function updateProgressBar(n, amount){
	n.update({'type': 'success', 'message': '<strong>Success</strong>!', 'progress': 100, 'delay': 1000});
}
...
JSONeditor.treeBuilder={
	...
	jsonSort:function(f){
		var textareaInput = f.jvalue.value.split("\r").join("");
		try{
			textareaInput = textareaInput.replace(/\\n/g,"");
			var inp = JSON.parse(textareaInput);
			if(inp instanceof Object && !Array.isArray(inp)){
				var obj = sortObject(inp);
				prettyPrint(JSON.stringify(obj));
			} else if(Array.isArray(inp)){
				prettyPrint(JSON.stringify(inp.sort()));
			}
		}catch(e){
			if(!e instanceof SyntaxError){
	...
	jsonChange:function(f,remove,rename){	
		try {
			$('#saveBtn').addClass('animated bounceIn');
			$('.jsontxtArea').addClass('animated fadeIn');
			var n = generateNotifyProgressBar();
			...
			v=JSON.stringify(prettyPrint(v))
			if(f.jtype.value=="string"){
				v=this.JSONstring.make(v)
			}
			
			updateProgressBar(n, 100);
			if(l=="json"){
				eval("v="+v)
		...
		$('#saveBtn').removeClass('animated bounceIn');
		$('.saveBtn').removeClass('animated fadeIn');
		return false
	},
